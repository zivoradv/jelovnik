...
/**
 * Tema "topla kuhinja" - inspirisana domaćim kuvanjem:
 * bordo crvena, topli krem tonovi, mekane senke i zaobljene ivice.
 */

// Osnovna paleta izvedena iz logotipa (topla crvena / bordo).
const BRAND = {
  50: '#FDF3F2',
  100: '#FAE3E1',
  200: '#F3C4C0',
...
// Topli akcenat (ćilibar / paprika) za sporedne naglaske.
const ACCENT = {
  200: '#F5D9A8',
  300: '#EDBF73',
  400: '#DFA04A',
...
// Neutralne boje su namerno tople (ka smeđoj), ne hladno sive.
const SAND = {
  25: '#FCFAF7',
  50: '#F8F4EF',
  100: '#F0E9E1',
...
// Mekane, toplo tonirane senke (MUI podrazumevano koristi hladno sive).
const warmShadow = (y: number, blur: number, a: number) =>
  `0px ${y}px ${blur}px 0px rgba(74, 47, 34, ${a})`;
...
const shadows = [
  ...
  ...Array.from(
    { length: 16 },
    (_, i) =>
      `${warmShadow(6 + i, 16 + i * 2, 0.1)}, ${warmShadow(16 + i * 2, 36 + i * 3, 0.08)}`,
  ),
...
const theme = createTheme({
  ...
  typography: {
    ...
    // Naslovi koriste meki serif - daje topao, "domaći" karakter.
    h1: {
