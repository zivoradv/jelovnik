...
import { mealHasOrders, repriceFutureOrders, usersWithUpcomingOrdersOfMeal } from './orders.service'
 ...
 * Nova cena važi za dane posle današnjeg (porudžbine se preračunaju); prošli dani ostaju kako su obračunati.
 */
export async function updateMeal(id: number, data: Partial<NewMeal>): Promise<Meal | undefined> {
    ...
    const priceChanged = Number(before.price) !== Number(updated.price)
    if (priceChanged) await repriceFutureOrders()
    ...
    if (relevantChange) {
        ...
        for (const a of affected) {
            await notifyUsers([a.userId], {
                ...
                body: deactivated
                    ? `Naručio/la si ga za: ${datesText(a.dates)}. Proveri porudžbinu i izaberi drugo jelo ako treba.`
                    : `Sada je „${updated.name}” po ceni ${Number(updated.price).toLocaleString('sr-RS')} RSD. Tvoja porudžbina za ${datesText(a.dates)} ostaje${priceChanged ? ' (nova cena važi od sutra)' : ''}.`,
                link: '/',
            })
        }
    }
...
/**
 * Brisanje jela. Jelo koje je iko ikada naručio ne može da se obriše – sa njim bi nestala istorija porudžbina
 * i dugova. Takvo jelo se samo deaktivira (nestaje iz ponude, račun ostaje).
 */
export async function deleteMeal(id: number): Promise<void> {
    const meal = await getMealById(id)
    if (!meal) return
    if (await mealHasOrders(id)) {
        throw new Error('Ovo jelo ima porudžbine, pa ne može da se obriše (nestao bi trag u dugovima). Umesto toga ga deaktiviraj.')
    }
